...
class SlidingWindowSolutions:
    ...
    def minSubArrayLen(self, target: int=7, nums: List[int]=[2,3,1,2,4,3]) -> int:
        ...
        for end in range(n):
            window_sum += nums[end]
            """
            Esto va haciendo el subarray mas grande hasta que el sum sea mayor o igual a target Window sum se               hará más pequeño (al quitarle el primer elemento del subarray, esto se hará hasta que window_sum                sea mayor a target)
